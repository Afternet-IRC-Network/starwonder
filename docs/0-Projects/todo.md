...
- [ ] **8. Reputation & alignment** — *open design question:* sector-local reputation so word doesn't travel far (your original idea), vs. faction reputation (a few factions whose standing gates missions / unlocks, from the old roadmap), vs. layering both — possibly over a good / evil axis. Whatever the model, it shapes how NPCs and other players treat you and which missions you can take. The **local** branch gets a concrete home in the idle-narrative design (per-station `standing`). → [idle-narrative.md](starwonder-mvp/idle-narrative.md)
- [ ] **8b. Idle narrative (docked downtime)** — while docked, a cheap lazy sim (settles on read, like Energy) rolls dice from a single **plugin registry**, nudges a small set of base stats (local standing, heat, prices, credits), and an AI narrates the deterministic event log — coloured by your trader's persona, the station's vibe, and a **goal** you set. Realises #8's local-reputation branch and is a self-directed cousin of #6's missions. → [idle-narrative.md](starwonder-mvp/idle-narrative.md)
